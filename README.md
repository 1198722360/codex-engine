...
The five long-running Compose services are the gateway, account manager, parent relay, MySQL, and Redis. One short-lived `init` service initializes credentials and the private CA after `deploy.sh` pulls the Engine image. Persistent deployment state is stored in directories beside `docker-compose.yml` (`gateway-state/`, `manager-state/`, `engine-bootstrap/`, `mysql-secrets/`, `redis-secrets/`, `private-ca/`, `public-ca/`, `mysql-data/`, `redis-data/`, `gateway-capture/`, `gateway-resources/`, and `account-volumes/`). The account manager creates per-account Engine volumes backed by `account-volumes/`; those containers are not static Compose services.
...
`deploy.sh` first removes only a legacy `engine-image` one-shot container by its Compose labels, then runs `docker compose pull` from the package directory. On a first deployment with no release manifest, it pulls `ghcr.io/1198722360/codex-engine-engine:latest`; on an existing deployment it reads the platform-specific Engine config digest registered under `manager-state/releases/`, restores the `latest` tag to that local image when present, and then runs `docker compose up -d --remove-orphans`. If the registered image was pruned, the script stops before startup with a bilingual message asking you to restore it or use the controlled upgrade flow. This prevents a mutable tag from entering an existing release without migration. The label cleanup does not remove volumes or runtime services. The one-shot `init` service performs the identity, CA, and release checks inside the Gateway image. Keep the supplied `start-redis.sh` beside `docker-compose.yml`; Compose mounts it read-only into the Redis container for password validation and configuration generation. The first start stores the two configured access credentials and creates the listed local directories. It also registers a disabled **Initial account**; it does not authorize that account automatically. To rotate either access credential later, edit `.env` and rerun `./deploy.sh`. Sign in again with the new web password, and update clients that use the old API key.
...
Re-running `./deploy.sh` retains deployment directories and long-running services while updating the Gateway and other Compose images. When a release manifest already exists, the script preserves its registered Engine config digest; a normal redeploy neither pulls a new Engine nor rewrites that manifest. If the old Engine image is absent locally, deployment stops before `init` starts and asks you to restore it or use the controlled upgrade flow. An Engine update requires request draining, state migration, replacement, and commit; pulling `:latest` is not an upgrade path. MySQL and Redis version tags are maintained upstream and may change on a later pull.
